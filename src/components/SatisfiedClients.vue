<template>
  <div class="container-fluid clients p-5">
      <div class="row clients-header text-start">
          <div class="col-12">
            <h2 class="fw-bold mb-3">Our Most Satisfied Clients</h2>
            <p>We cooperate with our clients throughout the whole project building process and we negotiate to fit with our client's ratification. Vestibulum auctor ornare leo, non suscipit magna</p>
          </div>
      </div>

      <div class="clients-logos mt-3">
          <div class="d-flex justify-content-between">
              <img src="../assets/logos/Kia-logo.png" width="150px" height="70px" alt="logo1">
              <img src="../assets/logos/Samsung.png" width="150px" height="70px" alt="logo2">
              <img src="../assets/logos/Vodafone_Logo.png" width="150px" height="70px" alt="logo3">
              <img src="../assets/logos/SEGA_logo.png" width="150px" height="70px" alt="logo4">
              <img src="../assets/logos/burger-king.png" width="150px" height="70px" alt="logo4">
          </div>
      </div>

      <div class="row clients-images text-start mt-4">
          <div class="col-sm-12 col-md-6 col-lg-4 my-2">
              <div class="pt-4 pb-2">
                  <img src="../assets/imgs/cropout-sample.avif" class="logo-image bg-secondary-subtle rounded py-3 px-3" alt="web dev">
              </div>
              <p class="fw-bold fs-5">Realtime Image Cropout</p>
              <p class="">Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.</p>
              <button class="button">View Project</button>
          </div>
          <div class="col-sm-12 col-md-6 col-lg-4 my-2">
              <div class="pt-4 pb-2">
                  <img src="../assets/imgs/cropout-sample.avif" class="logo-image bg-secondary-subtle rounded py-3 px-3" alt="app dev">
              </div>
              <p class="fw-bold fs-5">Realtime Image Cropout</p>
              <p class="">Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.</p>
              <button class="button">View Project</button>
          </div>
          <div class="col-sm-12 col-md-6 col-lg-4 my-2">
              <div class="pt-4 pb-2">
                  <img src="../assets/imgs/cropout-sample.avif" class="logo-image bg-secondary-subtle rounded py-3 px-3" alt="maintain">
              </div>
              <p class="fw-bold fs-5">Realtime Image Cropout</p>
              <p class="">Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.</p>
              <button class="button">View Project</button>
          </div>
          <div class="col-sm-12 col-md-6 col-lg-4 my-2">
              <div class="pt-4 pb-2">
                  <img src="../assets/imgs/cropout-sample.avif" class="logo-image bg-secondary-subtle rounded py-3 px-3" alt="marketing">
              </div>
              <p class="fw-bold fs-5">Realtime Image Cropout</p>
              <p class="">Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.</p>
              <button class="button">View Project</button>
          </div>
      </div>
  </div>
</template>

<script>
export default {

}
</script>

<style>
.clients{
  background: #4d4a4a;
  color: #fff;
}

.logo-image{
  width: 100%;
  height: 200px;
}

.button{
    background-color: #000;
    color: #fff;
    border: none;
    font-size: 15px;

    padding: 5px 20px; 
}
</style>